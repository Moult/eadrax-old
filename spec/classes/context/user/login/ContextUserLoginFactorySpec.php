...
class DescribeContextUserLoginFactory extends \PHPSpec\Context
{
    public function itIsAFactory()
    {
        $factory = Mockery::mock('Context_User_Login_Factory');
        $this->spec($factory)->should->beAnInstanceOf('Context_Factory');
    }

    public function itShouldCreateAContext()
    {
        $factory = Mockery::mock('Context_User_Login_Factory[model_user,module_auth]');
        ...
        $this->spec($context)->should->beAnInstanceOf('Context_User_Login');
    }

    public function itShouldAutoAllocateDataToModels()
    {
        $factory = new Context_User_Login_Factory(array(
            'username' => 'username',
            'password' => 'password'
        ));
