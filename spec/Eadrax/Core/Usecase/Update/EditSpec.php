...
class EditSpec extends ObjectBehavior
{
     ...
     * @param Eadrax\Core\Usecase\Update\Edit\Repository $update_add
     ...
     */
    function let($text, $update_add, $authenticator, $emailer, $formatter, $validator)
    {
        $data = array(
            'update' => $text
        );
    ...
    {
        $this->shouldHaveType('Eadrax\Core\Usecase\Update\Edit');
    }

    function it_fetches_the_interactor_with_text_data()
    {
        $this->fetch()->shouldHaveType('Eadrax\Core\Usecase\Update\Edit\Interactor');
    }
