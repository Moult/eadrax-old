...
class Register extends ObjectBehavior
{
    ...
    {
        $this->guest->shouldHaveType('Eadrax\Eadrax\Context\User\Register\Guest');
        $this->guest->shouldHaveType('Eadrax\Eadrax\Data\User');
        $this->guest->repository->shouldHaveType('Eadrax\Eadrax\Context\User\Register\Repository');
    ...
    function it_catches_authorisation_exceptions_during_usecase($data_user, $repository, $entity_auth, $entity_validation)
    {
        ...
        $this->execute()->shouldBe(array(
            ...
            'type' => 'authorisation',
            'data' => array(
                'errors' => array('Logged in users cannot register new accounts.')
            )
        ));
    }
